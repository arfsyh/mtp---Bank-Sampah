...
class Tulisan extends CI_Controller{
	...
	function add_tulisan(){
		...
		$this->load->view('admin/v_add_tulisan');
	}
	function get_edit(){
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
	                        ...
	                        $gambar=$gbr['file_name'];
													...
													$p=$user->row_array();
													$username=$p['username'];
													$this->m_tulisan->simpan_tulisan($judul,$isi,$imgslider,$username,$gambar,$slug);
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
	                        ...
	                        $judul=strip_tags($this->input->post('xjudul'));
													...
													//$imgslider=$this->input->post('ximgslider');
													$imgslider='0';
													$kode=$this->session->userdata('user_nama');
													$user=$this->m_pengguna->get_pengguna_login($kode);
													$p=$user->row_array();
													$username=$p['username'];
													$this->m_tulisan->update_tulisan($tulisan_id,$judul,$imgslider,$username,$gambar,$slug);
	                    ...
	                    redirect('admin/pengguna');
									...
									$p=$user->row_array();
									$username=$p['username'];
									$this->m_tulisan->update_tulisan_tanpa_img($tulisan_id,$judul,$isi,$imgslider,$username,$slug);
									echo $this->session->set_flashdata('msg','info');
									redirect('admin/tulisan');
	            }
